...
interface BackgroundTypingAnimationProps {
  opacity: string;
  delay: number;
  speed: number;
...
const BackgroundTypingAnimation: React.FC<BackgroundTypingAnimationProps> = ({
  ...
  useEffect(() => {
    const generateRandomText = () => {
      const length = Math.floor(Math.random() * 30 + 10); // Random length between 10 and 40
...
const LayeredTypingAnimation: React.FC = () => {
  return (
...
const TypingAnimation: React.FC = () => {
  ...
  const getLineStyle = (index: number, line: string) => {
    ...
    if (line === currentSet[index]) {
      if (line.toLowerCase().includes('error')) return 'bg-red-600/20 text-red-400';
      if (line.includes('installing...') || line.includes('building...')) return 'bg-yellow-600/20 text-yellow-400';
    }
    return '';
  };
      ...
      <div className="p-4 font-mono text-sm">
        {lines.map((line, index) => (
          <div key={index} className={`h-[32px] flex items-center ${getLineStyle(index, line)}`}>
            ...
            {currentLine === index && !isDeleting && <span className="animate-pulse ml-1">▋</span>}
          </div>
        ))}
      </div>
    </div>
